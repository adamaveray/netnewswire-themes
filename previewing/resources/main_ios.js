/** Via https://github.com/Ranchero-Software/NetNewsWire/tree/main/iOS/Resources */
...
function highlightRects(rects, clearOldRects = true, makeHighlightRect = makeHighlightRect) {
  ...
  if (container && clearOldRects) container.remove();
  ...
  return Array.from(rects, (rect) => container.appendChild(makeHighlightRect(rect, -top, -left)));
}
...
function scrollParent(node) {
  ...
  while (elt) {
    if (elt.scrollHeight > elt.clientHeight) return elt;
...
class FindState {
  constructor(options) {
    ...
    if (!regex) text = escapeRegex(text);
...
selectNextResult = withEncodedArg((options) => {
  if (CurrentFindState) CurrentFindState.selectNext();
});

selectPreviousResult = withEncodedArg((options) => {
  if (CurrentFindState) CurrentFindState.selectPrevious();
});
