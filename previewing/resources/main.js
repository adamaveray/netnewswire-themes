...
function wrapFrames() {
  document.querySelectorAll('iframe').forEach((element) => {
    if (element.height > 0 || parseInt(element.style.height) > 0) return;
...
function stripStyles() {
  ...
  document
    ...
    .forEach((element) =>
      stripStylesFromElement(element, ['color', 'background', 'font', 'max-width', 'max-height', 'position']),
    );
}
...
var ElementUnwrapper = {
  ...
  unwrapAppropriateChildren: function (elements) {
    ...
    else if (typeof elements === 'string') elements = document.querySelectorAll(elements);
